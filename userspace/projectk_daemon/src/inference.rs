...
static SCRATCH: OnceLock<SyncCell<[f32; SCR_TOT]>> = OnceLock::new();

fn scratch_buf() -> &'static mut [f32; SCR_TOT] {
    let cell: &SyncCell<[f32; SCR_TOT]> =
        SCRATCH.get_or_init(|| SyncCell(UnsafeCell::new([0.0; SCR_TOT])));
...
static MODEL: OnceLock<Box<Model>> = OnceLock::new();
...
pub fn init() {
    if load_weights(PKK_EMBEDDED) {
        eprintln!("projectk_daemon: Project-K 1.6MB INT4 online ({} KB, vocab={})",
            PKK_EMBEDDED.len() / 1024, PKK_VOCAB_SIZE);
    } else {
        eprintln!("projectk_daemon: weight load failed");
    }
}
...
fn load_weights(data: &[u8]) -> bool {
    ...
    if &data[0..4] != b"MHSI" {
        eprintln!("projectk_daemon: bad magic (expected MHSI)");
        return false;
    }
    let mut off = 4usize;
    ...
    if vocab != VOCAB || d != D || n_lay != N_LAYERS || dh0 != DH0 || dh1 != DH1 || gs != GROUP_SZ {
        eprintln!("projectk_daemon: header mismatch v={} d={} l={} dh0={} dh1={} gs={}",
    ...
    unsafe {
        let model = Box::new(Model { emb, blocks, lnf_w, lnf_b, pos_emb, vocab, d });
        MODEL.set(model).ok();
    }
...
fn fastrand() -> f64 {
    ...
    if seed == 0 {
        seed = (std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_millis() as u64).wrapping_mul(6364136223846793005).wrapping_add(1);
    }
...
pub fn report() -> String {
    if !LOADED.load(Ordering::Acquire) {
        return format!("Project-K: not loaded");
    }
    format!("Project-K INT4 nano: vocab={} d={} layers={} gs={} gen_count={}",
